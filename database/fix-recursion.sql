-- FIX: infinite recursion in committees policy
-- The SELECT policy was joining committee_members -> committees -> committee_members (loop)
...
-- ── COMMITTEES: simple, no cross-table joins ──────────────────
...
SELECT 'Recursion fixed! All policies now use simple auth.uid() IS NOT NULL.' AS status;
